...
# ============================================================
# RACE ENGINEER - LLM ANALYSIS v3.10.4 / DeepSeek provisional v1
# ============================================================
#
# Diseñado para:
#
...
def clean_driver_action_episode(
    ...
    result = {
        ...
        # Throttle 1.2: observacional. No se envía al LLM en v3.10.4.
...
# ============================================================
# DIRECCIÓN FACTUAL EXPLÍCITA v3.10.4
# ============================================================
...
def build_deterministic_grounded_episode_fallback(episode):
    ...
    En v3.10.4 también se usa como fuente de reparación selectiva: si una
...
def repair_invalid_episode_semantic_fields(response, episode, errors):
    """
    Reparación selectiva v3.10.4 antes de gastar un retry del LLM.
    ...
    if not repairs["replaced_fields"] and not bad_hypotheses:
        return None, {}
        ...
        for direction, direction_rows in by_direction.items():
            ...
            for row in direction_rows:
                ...
                classification_score = (
                    2 if row.get("classification") == "PRIORITARIO"
                    else 1 if row.get("classification") == "SECUNDARIO"
                    else 0
                )
                action_loss = abs(safe_float(row.get("action_time_loss_s")) or 0.0)
                score = (classification_score, action_loss)
...
# ============================================================
# PRIORIDAD DE SESIÓN POR RECURRENCIA v3.10.4
# ============================================================

SESSION_PRIORITY_POLICY_VERSION = "1.1"
...
- next_session_priorities es propiedad exclusiva de Python y NO forma
  parte de tu respuesta;
...
- si el item contiene input_sequence, esa relación temporal es parte del
  coaching autorizado: preservá el ORDEN de los inputs y no la colapses en una
  lista plana de cambios independientes;
- por ejemplo, si Python entrega una secuencia freno → acelerador, redactá el
  coaching conservando "primero freno, después acelerador" sin inventar causas,
  fases de curva ni distancias;
...
# ============================================================
# PRIORIDADES GLOBALES DETERMINISTAS v3.10.4
# ============================================================
...
def deterministic_priority_for_plan_item(
    ...
    El LLM ya no vuelve a generar estos datos en v3.10.4.
    """
    if not isinstance(
        item,
        dict,
...
def global_correction_instructions(errors):
    ...
    En v3.10.4 las prioridades numéricas/direccionales son propiedad de Python. Este bloque sólo corrige campos narrativos del LLM.
    """
    instructions = []
...
- si input_sequence no está vacío, conservá explícitamente el orden de esa
  secuencia en la redacción;
- no transformes una secuencia temporal en una lista plana de inputs;
...
- cada item debe conservar exactamente los observed_differences de ESA zona;
- no traslades dirección/freno/acelerador desde otra zona;
- no inviertas mayor/menor respecto de los hechos suministrados por Python;
...
# ============================================================
# CONSISTENCIA DIRECCIONAL GLOBAL v3.10.4
# ============================================================
...
def validate_global_zone_list_consistency(
    ...
    """
    Invariante global por zona v3.10.4.
...
def prune_only_invalid_global_list_items(
    ...
    """
    Fallback determinista v3.10.4 para listas narrativas opcionales globales.
...
def get_validated_global_response(
    ...
    if pruned_response is not None:
        ...
        print(
            "Síntesis global: fallback determinista v3.10.4 "
            f"aplicado; se descartaron {removed_count} items "
            "opcionales no grounded."
        )
...
def save_result(
    ...
    output_path = os.path.join(
        output_dir,
        stem + f"_llm_analysis_v3_10_4_deepseek_v2_{MODEL_NAME}.json",
    )
...
def main():
    ...
    print_header(
        "RACE ENGINEER - LLM ANALYSIS v3.10.4 / DeepSeek provisional v2"
    )
    ...
    for comparison_index, comparison in enumerate(
        ...
        if not detected_episode_catalog:
            raise RuntimeError(
                ...
                "La v3.10.4 requiere analyze_telemetry v3.8 "
        ...
        if episode_catalog:
            print(
                "Solicitando interpretación aislada + ranking comparativo v3.10.4..."
            )
