...
# ============================================================
# RACE ENGINEER - LLM ANALYSIS v3.10.8.5.1 / DeepSeek provisional v1
# ============================================================
#
# Diseñado para:
#
...
def build_deterministic_comparison_summary(
    ...
    # v3.10.8.5.1: una recomendación puede ser válida a nivel de episodio
...
def get_validated_comparison_summary_response(
    ...
    if deterministic_summary is not None:
        print(
            "    Resumen: fallback determinista v3.10.8.5.1 aplicado; "
...
def _point_anchored_profile(item, source_data, channel):
    """
    v3.10.8.5.1: un onset/release autorizado arrastra la forma del MISMO evento
...
def _sanitize_recurrence_regions(regions):
    ...
    v3.10.8.5.1: NO muta los dicts originales. priority_findings y
...
def _session_plan_sort_key(
    ...
    """
    v3.10.8.5.1: prioridad por especificidad + calidad del hallazgo.
...
def _comparison_quality_diagnostics(comparison):
    """
    v3.10.8.5.1 — diagnóstico determinista para confirmar o rechazar un
...
def _plan_item_has_actionable_coaching(item):
    ...
    if any(str(value or "").strip() for value in (item.get("targets", []) or [])):
        return True
    # v3.10.8.5.1: un hallazgo PRIORITARIO puede llegar al plan sólo por
...
def save_result(
    ...
    output_path = os.path.join(
        output_dir,
        stem + f"_llm_analysis_v3_10_8_5_1_deepseek_v2_{MODEL_NAME}.json",
    )
...
def main():
    ...
    print_header(
        "RACE ENGINEER - LLM ANALYSIS v3.10.8.5.1 / DeepSeek provisional v2"
    )
    ...
    for comparison_index, comparison in enumerate(
        ...
        if not detected_episode_catalog:
            raise RuntimeError(
                ...
                "La v3.10.8.5.1 requiere analyze_telemetry v3.8 "
        ...
        if not session_plan_eligible:
            ...
            print(
                "Solicitando interpretación aislada + ranking comparativo v3.10.8.5.1..."
            )
