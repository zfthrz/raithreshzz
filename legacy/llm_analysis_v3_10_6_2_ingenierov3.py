...
# ============================================================
# RACE ENGINEER - LLM ANALYSIS v3.10.6.2
# ============================================================
#
# Diseñado para:
#
...
GLOBAL_NUM_PREDICT_SCHEDULE = (1536, 2048, 3072)
...
def clean_driver_action_episode(
    ...
    result = {
        ...
        # Throttle 1.2: observacional. No se envía al LLM en v3.10.6.2.
...
# ============================================================
# DIRECCIÓN FACTUAL EXPLÍCITA v3.10.6.2
# ============================================================
...
def build_deterministic_grounded_episode_fallback(episode):
    ...
    En v3.10.6.2 también se usa como fuente de reparación selectiva: si una
...
def repair_invalid_episode_semantic_fields(response, episode, errors):
    """
    Reparación selectiva v3.10.6.2 antes de gastar un retry del LLM.
...
# ============================================================
# PERFIL DE ACCIÓN DE REFERENCIA v3.10.6.2
# ============================================================
...
def _coaching_target_for_channel_direction(
    ...
    fallback = {
        # v3.10.6.2: throttle mixed se completa desde reference_action_profile.
...
# ============================================================
# PRIORIDAD DE SESIÓN POR RECURRENCIA v3.10.6.2
# ============================================================
...
def build_session_coaching_facts(
    ...
    # v3.10.6.2: el target mixed de acelerador sólo existe si Python puede
...
# ============================================================
# PRIORIDADES GLOBALES DETERMINISTAS v3.10.6.2
# ============================================================
...
def deterministic_priority_for_plan_item(
    ...
    El LLM ya no vuelve a generar estos datos en v3.10.6.2.
    """
    if not isinstance(
        item,
        dict,
...
def global_correction_instructions(errors):
    ...
    En v3.10.6.2 las prioridades numéricas/direccionales son propiedad de Python. Este bloque sólo corrige campos narrativos del LLM.
    """
    instructions = []
...
# ============================================================
# CONSISTENCIA DIRECCIONAL GLOBAL v3.10.6.2
# ============================================================
...
def validate_global_zone_list_consistency(
    ...
    """
    Invariante global por zona v3.10.6.2.
...
def repair_global_optional_list_overflow(
    ...
    """
    Reparación determinista v3.10.6.2 para exceso de items en listas globales.
...
def prune_only_invalid_global_list_items(
    ...
    """
    Fallback determinista v3.10.6.2 para listas narrativas opcionales globales.
...
def get_validated_global_response(
    ...
    for attempt in range(
        ...
        global_num_predict = _global_num_predict_for_attempt(
            attempt
        )
        ...
        try:
            ...
            truncated = _looks_like_truncated_json(
                raw,
                exc,
            )
            ...
            if truncated:
                errors = [
                    "LLM_JSON_TRUNCATED: la respuesta anterior quedó "
                    "incompleta. Devolvé un JSON completo y conciso, "
                    "cerrando todas las cadenas, arrays y el objeto raíz."
                ]
                if attempt < MAX_GLOBAL_LLM_VALIDATION_ATTEMPTS:
                    next_budget = _global_num_predict_for_attempt(
                        attempt + 1
                    )
                    print(
                        "Síntesis global: JSON truncado; "
                        f"se amplía num_predict a {next_budget} "
    ...
    if pruned_response is not None:
        ...
        print(
            "Síntesis global: fallback determinista v3.10.6.2 "
            f"aplicado; se descartaron {removed_count} items "
            "opcionales no grounded."
        )
...
def save_result(
    ...
    output_path = os.path.join(
        output_dir,
        stem + f"_llm_analysis_v3_10_6_{MODEL_NAME}.json",
    )
    ...
    for comparison_index, comparison in enumerate(
        ...
        if not detected_episode_catalog:
            raise RuntimeError(
                ...
                "La v3.10.6.2 requiere analyze_telemetry v3.8 "
        ...
        if episode_catalog:
            print(
                "Solicitando interpretación aislada + ranking comparativo v3.10.6.2..."
            )
