...
def evaluate_candidate(
    ...
    *,
    ...
    # Weather v0.1: never infer semantic equivalence. When target weather is known,
    # candidate raw weather must be known and exactly equal.
    target_weather = norm_text(target.get("weather_conditions"))
    cand_weather = norm_text(candidate.get("weather_conditions"))
    if target_weather is not None:
        if cand_weather is None:
            reasons.append("WEATHER_UNKNOWN_WHILE_TARGET_KNOWN")
        elif cand_weather != target_weather:
...
def main() -> int:
    ap = argparse.ArgumentParser(
        description="H4 v0.1: selecciona benchmark histórico determinista para una sesión target."
    )
    ...
    try:
        ...
        target_weather_known = norm_text(target.get("weather_conditions")) is not None
        ...
        payload = {
            "metadata": {
                ...
                "hard_context_gate": [
                    ...
                    "exact_weather_raw_when_target_weather_known",
                ],
                "non_gating_observations_v0_1": [
                    "session_type",
                    "lmu_session_type",
                    "setup_sha256",
                ],
            ...
            "selection_scope": (
                "FULLY_GATED_V0_1" if target_weather_known
