...
- The release requirement now covers the complete public interface in Español and
  English, including the separate History browser and dynamic/empty/error states.
- The current code has 291 presentation update sites across the main GUI and History;
  the existing English support covers generated debriefs but not application chrome.
- R3c is therefore open. It will keep internal identifiers stable, bind the public
  language choice to interface plus future debriefs, and add a residual Spanish-text
  audit before claiming completion.
- No generated report, telemetry, History row, profile or coaching authority will be
  translated or regenerated by this work.
...
- GUI v1.51 adds lazy read-only general/monthly History statistics. Valid laps and
  ...
  valid laps and 2711.3 km without slowing application startup.
- Telemetry zone selection now presents validated H5.3d v0.2 steering morphology
...
- H2 queue retention gate: a newest pending human queue blocks creation of another
  full batch for that context. Read-only audit over the current workspace found
  31 batches across 11 contexts and 9 superseded unlabeled candidates totaling
  984.0 MiB; no files were moved or deleted.
...
It additionally records current/reference total steering variation, their delta
and exact sign-change counts. These trace-shape facts remain unclassified and do
not yet distinguish intentional input from correction.
