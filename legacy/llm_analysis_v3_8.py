...
# ============================================================
# RACE ENGINEER - LLM ANALYSIS v3.8
# ============================================================
#
# Diseñado para:
#
...
- La vuelta de referencia es la vuelta A.
- La vuelta comparada es la vuelta B.
- Un delta temporal positivo significa que B perdió tiempo.
- Un delta temporal negativo significa que B ganó tiempo.
...
- episodios de acción del piloto;
- propagación posterior de velocidad;
- ranking de prioridades.

NO vuelvas a calcular ni modificar ese ranking.

La unidad primaria para interpretar conducción es:

    driver_action_episode

Un driver_action_episode solamente puede ser creado a partir
de diferencias persistentes de:

- throttle;
- brake;
- steering_magnitude.

La velocidad NO se utiliza para unir acciones del piloto.

speed_propagation significa:

    diferencia de velocidad que continúa después de una acción.

Puede ser una CONSECUENCIA de una acción anterior.

NO la presentes automáticamente como:
- un nuevo error;
- una nueva acción;
- una nueva causa.
...
action_time_loss_s significa:
    ...
    cuánto cambió el delta temporal mientras el episodio
    de acción estuvo presente.

NO demuestra que la acción haya causado exactamente
esa cantidad de tiempo.

Forma correcta:

    "Durante la diferencia sostenida de acelerador,
    el delta aumentó 0.25 s."

Forma incorrecta:

    "El acelerador causó exactamente una pérdida de 0.25 s."

NO sumes action_time_loss_s de eventos internos.

NO sumes episodios que se solapan.

NO vuelvas a detectar los límites del episodio.
...
PROHIBIDO inferir problemas como:
...
salvo que los datos proporcionados tengan evidencia directa
suficiente para formularlo, y aun así debe presentarse
solamente como HIPÓTESIS.

No conviertas automáticamente:

- menos velocidad = error de conducción;
- más throttle = mejor salida;
- menos throttle = mala salida;
- más brake = frenada tardía;
- menos brake = frenada insuficiente;
- más steering = inestabilidad;
- steering distinto = mala trayectoria.

Interpretá las variables conjuntamente.
...
INTERPRETACIÓN:
lectura razonable respaldada por varias señales.

HIPÓTESIS:
explicación posible que no puede confirmarse.

Cuando haya speed_propagation:

explicá que la diferencia de velocidad persiste después
de la acción, pero no afirmes que el piloto siguió
cometiendo un error durante toda esa propagación.

Si un episodio solo contiene un canal de acción,
reducí la confianza de cualquier conclusión causal.

Si varios canales coinciden y el delta crece durante
el episodio, podés señalar que existe evidencia más fuerte
de una diferencia conductiva relevante.

Respondé en español.

Sé técnico, concreto y orientado al piloto.

No llenes espacio con recomendaciones genéricas.
"""
...
def build_comparison_prompt(
    ...
    if driver_episodes:
        ...
        mode_instruction = """
La fuente primaria es driver_action_episode_ranking.

Analizá los episodios EN EL ORDEN dado por Python.
No los reordenes.

Para cada episodio:

1. indicá cuánto aumentó/disminuyó el delta durante la acción;
2. identificá los action_channels;
3. explicá qué diferencias objetivas existen;
4. relacioná throttle/brake/steering con velocidad y RPM
   solamente cuando los datos estén presentes;
5. describí speed_propagation separadamente como consecuencia
   potencial;
6. formulá interpretación e hipótesis con el nivel de
   confianza adecuado.
"""
        ...
        mode_instruction = """
Este JSON no contiene driver_action_episode_ranking v3.8.

Usá loss_episode_ranking solamente como fallback.

No afirmes causalidad entre los canales de un episodio.
Indicá explícitamente esta limitación.
"""
...
TIEMPO A:
{format_seconds(comparison.get("reference_time_s"))}

VUELTA B - COMPARACIÓN:
{comparison_lap}

TIEMPO B:
{format_seconds(comparison.get("comparison_time_s"))}

DELTA REAL B - A:
{format_seconds(comparison.get("comparison_minus_reference_s"))}

DISTANCIA COMPARADA:
{comparison.get("distance_m")} m

PRIORIDAD DETERMINADA POR PYTHON:
...
- No modifiques el ranking.
- No vuelvas a construir zonas.
- No sumes los tiempos de los eventos.
- No conviertas speed_propagation en acción del piloto.
- No asumas que toda diferencia de velocidad fue causada
  dentro del punto donde se observa.
- No inventes nombres de curvas.
- Usá siempre las distancias suministradas.
- No presentes una hipótesis como hecho.
- Si una evidencia no alcanza, decilo.
- Si un episodio no tiene pérdida temporal positiva,
  no lo presentes como una pérdida principal.
- Priorizá los episodios que Python colocó primero.
...
(Repetir solamente para los episodios relevantes.)

## Patrón general de conducción
...
Python ya determinó el ranking de prioridad de las comparaciones.
NO lo reordenes arbitrariamente.

Las comparaciones marcadas como prioritarias para análisis
de conducción tienen más valor que vueltas muy lentas,
outliers o vueltas poco representativas.

No presentes speed_propagation como una acción adicional.

No inventes curvas ni sectores.

No inventes causas mecánicas.

No repitas todos los números.

Buscá patrones que aparezcan entre las comparaciones
prioritarias.
...
Si una hipótesis solamente aparece en una vuelta,
no la presentes como patrón recurrente.
