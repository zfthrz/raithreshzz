...
def select_queue(
    ...
    for lens_name, ranked_pairs in lenses.items():
        for rank, pair in enumerate(ranked_pairs[:per_lens], start=1):
            ...
            selected[pair_id]["selected_by"].append({"lens": lens_name, "rank": rank})
    ...
    if max_total is not None:
        ...
        queue = sorted(
            queue,
            key=lambda item: (
                -len(item["selected_by"]),
                min(ref["rank"] for ref in item["selected_by"]),
                item["pair_id"],
            ),
        )[:max_total]
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--per-lens", type=int, default=20)
...
def main() -> int:
    ...
    print()
    print("=" * 70)
    print("RACE ENGINEER - PAIR REVIEW QUEUE v1.0")
    print("=" * 70)
    print()
