...
# ============================================================
# RACE ENGINEER - LLM ANALYSIS v3.10.8.3
# ============================================================
#
# Diseñado para:
#
...
- las diferencias de magnitud de dirección son OBSERVACIONALES en esta
  versión y NO se convierten en una orden directa de volante.
...
- NO recomiendes aumentar, reducir, replicar ni ajustar el volante/dirección;
- steering_magnitude es observation-only hasta disponer de evidencia explícita
  de trayectoria/geométrica autorizada por Python.
...
def validate_comparison_llm_response(
    ...
    for index, assessment in enumerate(
        ...
        if not isinstance(
            ...
            if "steering_magnitude" in _channels_mentioned_in_text(recommendation):
                errors.append(
                    f"Episodio {episode_id}.recommendation: "
                    "steering_magnitude es observation-only; no puede "
                    "convertirse en target directo de volante/dirección."
                )
...
def validate_single_episode_llm_response(
    ...
    if not isinstance(recommendation, str):
        ...
        if "steering_magnitude" in _channels_mentioned_in_text(recommendation):
            errors.append(
                "recommendation: steering_magnitude es observation-only; "
                "no puede convertirse en target directo de volante/dirección."
            )
...
def validate_comparison_summary_llm_response(
    ...
    if not isinstance(conclusion, str):
        ...
        if "steering_magnitude" in _channels_mentioned_in_text(conclusion):
            errors.append(
                "conclusion: steering_magnitude es observation-only; "
                "no puede convertirse en target directo."
            )
...
def get_validated_comparison_summary_response(
    ...
    if deterministic_summary is not None:
        print(
            "    Resumen: fallback determinista v3.10.8.3 aplicado; "
...
def _point_anchored_profile(item, source_data, channel):
    """
    v3.10.8.3: un onset/release autorizado arrastra la forma del MISMO evento
...
def _sanitize_recurrence_regions(regions):
    ...
    v3.10.8.3: NO muta los dicts originales. priority_findings y
...
def _session_plan_sort_key(
    ...
    """
    v3.10.8.3: prioridad por especificidad + calidad del hallazgo.
...
def _comparison_quality_diagnostics(comparison):
    """
    v3.10.8.3 — diagnóstico determinista para confirmar o rechazar un
...
SESSION_ACTIONABILITY_POLICY_VERSION = "1.1"
...
def validate_global_llm_response(
        ...
        if isinstance(value, list):
            ...
            if field == "opportunities":
                ...
                for index, item in enumerate(value):
                    ...
                    if "steering_magnitude" in _channels_mentioned_in_text(item):
                        errors.append(
                            f"{field}[{index}]: steering_magnitude es "
                            "observation-only y no puede ser una oportunidad "
                            "de coaching directa."
                        )
    ...
    if not isinstance(
        ...
        if "steering_magnitude" in _channels_mentioned_in_text(conclusion):
            errors.append(
                "conclusion global: steering_magnitude es observation-only; "
                "no puede convertirse en target directo."
            )
        validate_temporal_observation_not_action_target(
            conclusion,
...
def save_result(
    ...
    output_path = os.path.join(
        output_dir,
        stem + f"_llm_analysis_v3_10_8_3_{MODEL_NAME}.json",
    )
    ...
    for comparison_index, comparison in enumerate(
        ...
        if not detected_episode_catalog:
            raise RuntimeError(
                ...
                "La v3.10.8.3 requiere analyze_telemetry v3.8 "
        ...
        if not session_plan_eligible:
            ...
            print(
                "Solicitando interpretación aislada + ranking comparativo v3.10.8.3..."
            )
