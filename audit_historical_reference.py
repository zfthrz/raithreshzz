...
def main() -> int:
    ap = argparse.ArgumentParser(description="Audita candidates H4 historical reference v0.2.")
    ...
    print("=" * 100)
    print("RACE ENGINEER - H4 HISTORICAL REFERENCE AUDIT v0.2")
    print("=" * 100)
    ...
    print(f"Weather: {target.get('weather_conditions')} / class={target.get('weather_class')}")
    ...
    print("session  lap   duration     delta_target  type        weather  same_setup  timestamp")
    for c in eligible:
        ...
        print(
            ...
            f"{str(c.get('session_type'))[:10]:10s} {str(c.get('weather_class'))[:7]:7s} "
            f"{str(obs.get('same_setup_sha256')):10s} {c.get('timestamp_utc')}"
        )
