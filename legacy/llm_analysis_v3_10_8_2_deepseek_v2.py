...
# ============================================================
# RACE ENGINEER - LLM ANALYSIS v3.10.8.2 / DeepSeek provisional v1
# ============================================================
#
# Diseñado para:
#
...
def get_validated_comparison_summary_response(
    ...
    if deterministic_summary is not None:
        print(
            "    Resumen: fallback determinista v3.10.8.2 aplicado; "
...
def _point_anchored_profile(item, source_data, channel):
    """
    v3.10.8.2: un onset/release autorizado arrastra la forma del MISMO evento
...
def _sanitize_recurrence_regions(regions):
    ...
    v3.10.8.2: NO muta los dicts originales. priority_findings y
...
SESSION_PRIORITY_POLICY_VERSION = "1.6"
...
def _session_plan_sort_key(
    ...
    """
    v3.10.8.2: prioridad por especificidad + calidad del hallazgo.
...
def build_driver_cues_for_plan_item(item, max_cues=2):
    ...
    if throttle_onset or throttle_release:
        ...
        if profile is not None:
            summary = str(profile.get("shape_summary") or "").strip()
            if summary:
                text += "; usá como forma de referencia: " + summary
        cue = {
            "channel": "throttle",
            "kind": "spatial_points",
            "text": text,
...
def save_result(
    ...
    output_path = os.path.join(
        output_dir,
        stem + f"_llm_analysis_v3_10_8_2_deepseek_v2_{MODEL_NAME}.json",
    )
...
def main():
    ...
    print_header(
        "RACE ENGINEER - LLM ANALYSIS v3.10.8.2 / DeepSeek provisional v2"
    )
    ...
    for comparison_index, comparison in enumerate(
        ...
        if not detected_episode_catalog:
            raise RuntimeError(
                ...
                "La v3.10.8.2 requiere analyze_telemetry v3.8 "
        ...
        if not session_plan_eligible:
            ...
            print(
                "Solicitando interpretación aislada + ranking comparativo v3.10.8.2..."
            )
