...
# Conservative automatic REJECT bound. The closest human DIFFERENT in the calibration set
# was 623.5 m apart with zero overlap. Distances inside this unobserved gap remain AMBIGUOUS.
REJECT_CENTER_MIN_M = 623.5
...
def classify_pair(pair: dict[str, Any]) -> dict[str, Any]:
    ...
    # Far, non-overlapping negatives only. We deliberately do not extrapolate into the
    # 45-623.5 m region that has no human boundary labels yet.
    if center >= REJECT_CENTER_MIN_M and overlap_union <= REJECT_OVERLAP_UNION_MAX + EPS:
        return {
            "decision": "REJECT",
            "rule_id": "FAR_ZERO_OVERLAP",
            "reasons": [
                f"center_diff_m={center:.3f}>=calibrated_reject_bound_{REJECT_CENTER_MIN_M}",
    ...
    if core_spatial_match:
        if weak_shape_conflict_veto(pair, shape):
            return {
                "decision": "AMBIGUOUS",
                "rule_id": "CORE_SPATIAL_BUT_WEAK_SHAPE_CONFLICT",
                "reasons": [
                    "core_spatial_match",
...
def main() -> int:
    ap = argparse.ArgumentParser(
        description="Matcher cross-session H2 v0.1 conservador: MATCH / AMBIGUOUS / REJECT."
    )
    ...
    payload = {
        "metadata": {
            ...
            "thresholds": {
                ...
                "reject_center_min_m": REJECT_CENTER_MIN_M,
