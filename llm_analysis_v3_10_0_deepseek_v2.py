...
# ============================================================
# RACE ENGINEER - LLM ANALYSIS v3.10.0 / DeepSeek provisional v1
# ============================================================
#
# Diseñado para:
#
...
def clean_driver_action_episode(
    ...
    result = {
        ...
        # Throttle 1.2: observacional. No se envía al LLM en v3.10.0.
...
# ============================================================
# DIRECCIÓN FACTUAL EXPLÍCITA v3.10.0
# ============================================================
...
# ============================================================
# PRIORIDAD DE SESIÓN POR RECURRENCIA v3.10.0
# ============================================================
...
# ============================================================
# PRIORIDADES GLOBALES DETERMINISTAS v3.10.0
# ============================================================
...
def deterministic_priority_for_plan_item(
    ...
    El LLM ya no vuelve a generar estos datos en v3.10.0.
    """
    if not isinstance(
        item,
        dict,
...
def global_correction_instructions(errors):
    ...
    En v3.10.0 las prioridades numéricas/direccionales son propiedad de Python. Este bloque sólo corrige campos narrativos del LLM.
    """
    instructions = []
...
# ============================================================
# CONSISTENCIA DIRECCIONAL GLOBAL v3.10.0
# ============================================================
...
def render_global_analysis(
    ...
    for pattern in braking_patterns[:3]:
        ...
        if magnitude is not None:
            ...
            point_summaries.append(
                f"Zona {label}: inicio de frenada {magnitude} m {move}"
            )
    ...
    for pattern in brake_release_patterns[:3]:
        ...
        if magnitude is not None:
            move = "más tarde" if direction == "later" else "más temprano"
            point_summaries.append(
                f"Zona {label}: liberación de freno {magnitude} m {move}"
            )
...
def save_result(
    ...
    output_path = os.path.join(
        output_dir,
        stem + f"_llm_analysis_v3_10_0_deepseek_v2_{MODEL_NAME}.json",
    )
...
def main():
    ...
    print_header(
        "RACE ENGINEER - LLM ANALYSIS v3.10.0 / DeepSeek provisional v2"
    )
    ...
    for comparison_index, comparison in enumerate(
        ...
        if not detected_episode_catalog:
            raise RuntimeError(
                ...
                "La v3.10.0 requiere analyze_telemetry v3.8 "
        ...
        if episode_catalog:
            print(
                "Solicitando interpretación aislada + ranking comparativo v3.10.0..."
            )
