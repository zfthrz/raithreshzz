...
def derive_context_from_analysis(analysis: dict[str, Any]) -> dict[str, Any]:
    ...
    variant = norm_text(vehicle.get("vehicle_variant"))
    car_name = norm_text(vehicle.get("car_name_raw"))

    # Older analyze_telemetry v3.8 JSONs may have context keys directly in metadata.
...
def validate_selection_target_against_analysis(
    ...
    if not isinstance(target, dict):
        raise ValueError("selection.target ausente/inválido.")
    ...
    if target_ref_lap is None:
        raise ValueError("selection.target.reference.lap ausente.")
    ...
    selection_context = selection.get("context") or {}
    checks = (
...
def build_dual_reference(
    ...
    *,
    ...
    selection_status = norm_text(selection.get("status"))
    ...
    if historical_available:
        ...
        historical_ref = {
            ...
            "same_setup_sha256": (
                (selected.get("compatibility_observations") or {}).get("same_setup_sha256")
                if isinstance(selected.get("compatibility_observations"), dict)
                else None
            ),
    ...
    return {
        ...
        "status": dual_status,
        "context": {
            "track": target.get("track") or (selection.get("context") or {}).get("track") or context.get("track"),
            "track_layout": target.get("track_layout") or (selection.get("context") or {}).get("track_layout") or context.get("track_layout"),
            "vehicle_variant": target.get("vehicle_variant") or (selection.get("context") or {}).get("vehicle_variant") or context.get("vehicle_variant"),
            "car_name_raw": target.get("car_name_raw") or (selection.get("context") or {}).get("car_name_raw") or context.get("car_name_raw"),
        },
        "target_session": {
...
def main() -> int:
    ap = argparse.ArgumentParser(
        description="H5.1 v0.1: construye contexto dual-reference determinista sin cambiar coaching."
    )
