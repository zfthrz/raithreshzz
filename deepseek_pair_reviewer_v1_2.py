...
Return one JSON object only, with exactly these fields:
{
  ...
  "decisive_evidence": ["short factual evidence statement", "..."],  # 1 to 4 items
  "reason": "brief explanation grounded only in supplied features"
}

Use at most 6 reason_codes and at most 4 decisive_evidence items. Keep reason under 900 characters.
...
def normalize_review_response(value):
    """Deterministic schema repair only; never changes label/confidence/reason semantics."""
...
def main():
    ...
    if args.dry_run:
        print("=" * 78)
        print("H2.2 - DEEPSEEK PAIR REVIEWER v1.1 DRY RUN")
        print("=" * 78)
    ...
    print("=" * 78)
    print("H2.2 - DEEPSEEK PAIR REVIEWER v1.1")
    print("=" * 78)
    print(f"Model: {MODEL_NAME}")
    ...
    for index, pair in enumerate(work, 1):
        ...
        for attempt in range(1, MAX_CLASSIFICATION_ATTEMPTS + 1):
            ...
            parsed, deterministic_repairs = normalize_review_response(parsed)
        ...
        if last_errors:
            ...
            if deterministic_repairs:
                review["deterministic_repairs"] = deterministic_repairs
