...
# ============================================================
# RACE ENGINEER - LLM ANALYSIS v3.10.3 / DeepSeek provisional v1
# ============================================================
#
# Diseñado para:
#
...
def clean_driver_action_episode(
    ...
    result = {
        ...
        # Throttle 1.2: observacional. No se envía al LLM en v3.10.3.
...
# ============================================================
# DIRECCIÓN FACTUAL EXPLÍCITA v3.10.3
# ============================================================
...
def build_deterministic_grounded_episode_fallback(episode):
    """
    Fallback factual mínimo v3.8.18.
...
# ============================================================
# PRIORIDAD DE SESIÓN POR RECURRENCIA v3.10.3
# ============================================================

SESSION_PRIORITY_POLICY_VERSION = "1.0"
...
def build_session_coaching_facts(
    ...
    # validado usando su intervalo agregado.
    enrich_items_with_track_location(
        repeated_throttle_onset_patterns,
        track_location_context,
    )
...
# ============================================================
# PRIORIDADES GLOBALES DETERMINISTAS v3.10.3
# ============================================================
...
def deterministic_priority_for_plan_item(
    ...
    El LLM ya no vuelve a generar estos datos en v3.10.3.
    """
    if not isinstance(
        item,
        dict,
...
def global_correction_instructions(errors):
    ...
    En v3.10.3 las prioridades numéricas/direccionales son propiedad de Python. Este bloque sólo corrige campos narrativos del LLM.
    """
    instructions = []
...
# ============================================================
# CONSISTENCIA DIRECCIONAL GLOBAL v3.10.3
# ============================================================
...
def validate_global_zone_list_consistency(
    ...
    """
    Invariante global por zona v3.10.3.
...
def prune_only_invalid_global_list_items(
    ...
    """
    Fallback determinista v3.10.3 para listas narrativas opcionales globales.
...
def get_validated_global_response(
    ...
    if pruned_response is not None:
        ...
        print(
            "Síntesis global: fallback determinista v3.10.3 "
            f"aplicado; se descartaron {removed_count} items "
            "opcionales no grounded."
        )
...
def save_result(
    ...
    output_path = os.path.join(
        output_dir,
        stem + f"_llm_analysis_v3_10_3_deepseek_v2_{MODEL_NAME}.json",
    )
...
def main():
    ...
    print_header(
        "RACE ENGINEER - LLM ANALYSIS v3.10.3 / DeepSeek provisional v2"
    )
    ...
    for comparison_index, comparison in enumerate(
        ...
        if not detected_episode_catalog:
            raise RuntimeError(
                ...
                "La v3.10.3 requiere analyze_telemetry v3.8 "
        ...
        if episode_catalog:
            print(
                "Solicitando interpretación aislada + ranking comparativo v3.10.3..."
            )
