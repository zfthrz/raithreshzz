...
# ============================================================
# RACE ENGINEER - LLM ANALYSIS v3.10.8.1 / DeepSeek provisional v1
# ============================================================
#
# Diseñado para:
#
...
def get_validated_comparison_summary_response(
    ...
    if deterministic_summary is not None:
        print(
            "    Resumen: fallback determinista v3.10.8.1 aplicado; "
...
SESSION_PRIORITY_POLICY_VERSION = "1.5"
...
def _session_plan_sort_key(
    ...
    """
    v3.10.8.1: prioridad por ESPECIFICIDAD de evidencia accionable.

    1) punto físico repetido (Braking Point 2.1 / Throttle Point 1.2.1);
    2) punto físico VALID individual autorizado;
    3) reference_action_profile concreto de freno/acelerador;
    4) resto de evidencia accionable.

    Dentro del mismo nivel, la recurrencia y la cantidad de comparaciones
    siguen siendo preferidas. El ranking individual y la pérdida local sólo
    desempatan. Steering y diferencias genéricas de nivel siguen siendo
    observation-only.
    """
    kind = item.get("kind")
    ...
    return (
        ...
        best_rank,
        -max_loss,
        start if start is not None else 999999.0,
    )
...
SESSION_COMPARISON_QUALITY_GATE_VERSION = "1.0"
...
def build_session_comparison_quality_gate(valid_comparison_results):
    """
    Excluye del agregado de coaching únicamente vueltas globalmente no
    representativas respecto del grupo de comparaciones de ESA sesión.

    La comparación completa se conserva en el JSON y sigue siendo auditable;
    sólo deja de alimentar recurrencia, patrones repetidos y next_stint_plan.

    Método conservador y robusto para muestras pequeñas:
      - mediana de |delta de vuelta|;
      - MAD -> sigma robusta;
      - exige superar simultáneamente un umbral robusto y uno relativo.

    No usa un corte absoluto del tipo "delta > N segundos".
    """
    rows = []
    ...
    if not rows:
        return {
            ...
            "method": "median_mad_plus_relative_outlier_gate",
            "comparison_count": 0,
            "included_count": 0,
            "excluded_count": 0,
            "comparisons": [],
    ...
    if enough:
        ...
        exclusion_threshold = max(robust_threshold, relative_threshold)
    else:
    ...
    for row in rows:
        is_outlier = bool(
            enough
            and exclusion_threshold is not None
            and row["abs_delta_s"] > exclusion_threshold
        )
        row["session_plan_eligible"] = not is_outlier
        row["quality_status"] = (
            "COACHING_EXCLUDED_NON_REPRESENTATIVE_LAP"
            if is_outlier
            else "SESSION_PLAN_ELIGIBLE"
        )
        row["reason"] = (
            "global_lap_delta_robust_outlier"
            if is_outlier
            else None
        )
        if is_outlier:
    ...
    return {
        ...
        "exclusion_threshold_s": exclusion_threshold,
        "excluded_comparisons": excluded,
        "comparisons": rows,
        "policy": (
            "comparison preserved in JSON; excluded only from session-level coaching aggregation"
        ),
    }
...
def build_session_coaching_facts(
    ...
    return {
        ...
        "session_priority_policy": {
            ...
            "comparison_quality_gate":
                "median_mad_plus_relative_outlier_gate",
...
- nunca conviertas steering_magnitude en una orden directa de aumentar/reducir/replicar volante;
...
SESSION_ACTIONABILITY_POLICY_VERSION = "1.0"
...
def build_driver_cues_for_plan_item(item, max_cues=2):
    """Construye como máximo dos cues driver-facing; steering nunca es target."""
    if not isinstance(item, dict):
        return []
    ...
    if throttle_onset or throttle_release:
        text = " y ".join(value for value in (throttle_onset, throttle_release) if value)
        cues.append({
            "channel": "throttle",
            "kind": "spatial_points",
            "text": text,
    ...
    for profile in (item.get("reference_action_profiles", []) or []):
        ...
        if channel in existing_channels or channel not in {"brake", "throttle"}:
            continue
        summary = str(profile.get("shape_summary") or "").strip()
        if not summary:
            continue
...
def render_global_analysis(
    ...
    for index, item in enumerate(plan[:3], start=1):
        ...
        if item.get("kind") == "repeated_region" and comparison_count:
            lines.append(
                f"**Confianza:** patrón repetido en {comparison_count} comparaciones válidas para el plan."
            )
            lines.append("")
...
def save_result(
    ...
    output_path = os.path.join(
        output_dir,
        stem + f"_llm_analysis_v3_10_8_1_deepseek_v2_{MODEL_NAME}.json",
    )
...
def main():
    ...
    print_header(
        "RACE ENGINEER - LLM ANALYSIS v3.10.8.1 / DeepSeek provisional v2"
    )
    ...
    for comparison_index, comparison in enumerate(
        ...
        if not detected_episode_catalog:
            raise RuntimeError(
                ...
                "La v3.10.8.1 requiere analyze_telemetry v3.8 "
        ...
        if not session_plan_eligible:
            ...
            print(
                "Solicitando interpretación aislada + ranking comparativo v3.10.8.1..."
            )
