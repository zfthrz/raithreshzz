...
GUI v1.55 exposes the complete global-keyboard contract inside the application.
`F1` and the visible sidebar action open a single non-modal shortcut reference;
`Escape` closes it together with the existing transient presentation surfaces.
This remains presentation-only and does not alter deterministic runtime authority.

GUI v1.56 applies consistent secondary notebooks to History, Statistics and
...
GUI v1.58 reflows the summary dashboard deterministically at workspace-width
boundaries: four columns, a two-by-two grid, then one stacked column. Visual previews
