...
# ============================================================
# RACE ENGINEER - LLM ANALYSIS v3.10.2 / DeepSeek provisional v1
# ============================================================
#
# Diseñado para:
#
...
def clean_driver_action_episode(
    ...
    result = {
        ...
        # Throttle 1.2: observacional. No se envía al LLM en v3.10.2.
...
FORBIDDEN_EXTERNAL_GROUNDING_PATTERNS = (
    ...
    (r"\bestabilidad\b|\binestabilidad\b", "estabilidad/dinámica del vehículo"),
...
# ============================================================
# DIRECCIÓN FACTUAL EXPLÍCITA v3.10.2
# ============================================================
...
def render_comparison_analysis(
    ...
    lines.append(
        ...
        f"{plain_seconds(comparison['comparison_time_s'])}, "
        f"{signed_seconds(comparison['comparison_minus_reference_s'])} "
        f"respecto de la referencia de "
        f"{plain_seconds(comparison['reference_time_s'])}."
    )
...
# ============================================================
# PRIORIDAD DE SESIÓN POR RECURRENCIA v3.10.2
# ============================================================
...
# ============================================================
# PRIORIDADES GLOBALES DETERMINISTAS v3.10.2
# ============================================================
...
def deterministic_priority_for_plan_item(
    ...
    El LLM ya no vuelve a generar estos datos en v3.10.2.
    """
    if not isinstance(
        item,
        dict,
...
def global_correction_instructions(errors):
    ...
    En v3.10.2 las prioridades numéricas/direccionales son propiedad de Python. Este bloque sólo corrige campos narrativos del LLM.
    """
    instructions = []
...
# ============================================================
# CONSISTENCIA DIRECCIONAL GLOBAL v3.10.2
# ============================================================
...
def prune_only_invalid_global_list_items(
    ...
    """
    Fallback determinista v3.10.2 para listas narrativas opcionales globales.
...
def get_validated_global_response(
    ...
    if pruned_response is not None:
        ...
        print(
            "Síntesis global: fallback determinista v3.10.2 "
            f"aplicado; se descartaron {removed_count} items "
            "opcionales no grounded."
        )
...
def render_global_analysis(
    ...
    if reference_lap is not None and reference_time is not None:
        lines.append(
            ...
            f"con {plain_seconds(reference_time)}."
        )
...
def save_result(
    ...
    output_path = os.path.join(
        output_dir,
        stem + f"_llm_analysis_v3_10_1_deepseek_v2_{MODEL_NAME}.json",
    )
...
def main():
    ...
    print_header(
        "RACE ENGINEER - LLM ANALYSIS v3.10.2 / DeepSeek provisional v2"
    )
    ...
    for comparison_index, comparison in enumerate(
        ...
        if not detected_episode_catalog:
            raise RuntimeError(
                ...
                "La v3.10.2 requiere analyze_telemetry v3.8 "
        ...
        if episode_catalog:
            print(
                "Solicitando interpretación aislada + ranking comparativo v3.10.2..."
            )
